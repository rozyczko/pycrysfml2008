...
module py_cfml_reflections
    ...
    function py_gener_reflections(self_ptr,args_ptr) result(resul) bind(c)
        ...
        integer, pointer :: p_iphase => null()
        integer, dimension(3), target :: seqindx
        integer, dimension(3,2), target :: hlim
        integer, dimension(:), pointer :: p_seqindx => null()
        integer, dimension(:,:), pointer :: p_hlim => null()
        logical :: is_spg,is_kinfo
        logical, target :: magext,order,unique,mag_only,friedel
        logical, pointer :: p_magext => null(),p_order => null(),p_unique => null(),p_mag_only => null(),p_friedel => null()
        character(len=:), allocatable :: fortran_type
        character(len=:), allocatable, target :: ref_typ
        character(len=:), pointer :: p_ref_typ
        character(len=1) :: ord
        class(cell_g_type), allocatable :: cell
        class(spg_type), allocatable, target :: spg
        class(spg_type), pointer :: p_spg => null()
        type(kvect_info_type) :: kout
        type(kvect_info_type), target :: kinfo
        type(kvect_info_type), pointer :: p_kinfo => null()
        ...
        ! Calling Fortran procedure
        if (ierror == 0) then
            call gener_reflections(cell,slmin,slmax,reflex,spg=p_spg,iphase=p_iphase,magext=p_magext,kinfo=p_kinfo,order=p_order,unique=p_unique,seqindx=p_seqindx,hlim=p_hlim,mag_only=p_mag_only,friedel=p_friedel,ref_typ=p_ref_typ,kout=kout) 
        end if
        if (ierror == 0) ierror = err_cfml%ierr
    ...
    function py_gener_reflections_shub(self_ptr,args_ptr) result(resul) bind(c)
        ...
        ! Calling Fortran procedure
        if (ierror == 0) then
            call gener_reflections_shub(cell,spg,smax,reflex,pfriedel)
        end if
        if (ierror == 0) ierror = err_cfml%ierr
        if (ierror == 0) call wrap_reflist_type(reflex,di_reflex,ierror)
